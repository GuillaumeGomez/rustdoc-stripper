...
const BASIC18_MD: &str = r#"<!-- fn compile_resources -->
Call from build script to run `glib-compile-resources` to generate compiled gresources to embed
in binary with [resources_register_include]. `target` is relative to `OUT_DIR`.
...
fn get_basic18_md(file: &str) -> String {
    format!(
        r#"<!-- file {} -->
{}"#,
        file, BASIC18_MD,
...
#[allow(unused_must_use)]
#[test]
fn test18_strip_failure() {
    let test_file = "basic18-strip.rs";
    let comment_file = "basic18-strip.md";
    let temp_dir = tempdir().unwrap();
    gen_file(&temp_dir, test_file, BASIC18);
    {
        let mut f = gen_file(&temp_dir, comment_file, "");
        stripper_lib::strip_comments(temp_dir.path(), test_file, &mut f, true);
    }
    compare_files(
        &get_basic18_md(test_file),
        &temp_dir.path().join(comment_file),
    );
